...
| 图像上传 | 服务端代理上传 | 前端 base64 → `/api/upload-file` → Google Cloud Storage |
| 图像分析 | 本地 Python 服务 | Flask + CLIP（openai/clip-vit-base-patch32），返回中文标签，**必须启动** |
| 故事生成 | DeepSeek API | 文本生成，基于分析结果 + 用户创意生成 6 页脚本 |
| 插画生成 | 即梦 4.0（火山引擎） | 异步任务（提交 → 轮询），AK/SK 签名 |
| 存储 | Google Cloud Storage | 上传图片与即梦生成图均为 GCS 公网 URL |
...
Next.js API (pages/api/)
    ├─ POST /api/upload-file      → 接收 base64，上传 GCS，返回 publicUrl
    ├─ POST /api/analyze-images   → multipart 接收图片，转发 Python 视觉服务，返回 summary + all_labels
    ├─ POST /api/generate-story   → 调用 DeepSeek，入参含 imageAnalysis / imageAnalysisDescription
    └─ POST /api/generate-image   → 调用即梦 4.0，入参 prompt / style / referenceImageUrl

外部服务
    ├─ Python 视觉服务 (Flask, port 5001)  ← 必须运行，无降级
    ├─ DeepSeek API
    ├─ 火山引擎即梦 4.0 API
...
├── pages/
│   ├── api/
│   │   ├── upload.ts           # 生成 GCS 签名 URL（当前未用，上传走 upload-file）
│   │   ├── upload-file.ts      # 接收 base64，写 GCS，返回 publicUrl，body 限制 20mb
│   │   ├── analyze-images.ts   # multipart 解析 images[]，调 lib/vision-python，无降级，失败 503
...
├── components/
│   └── PhotoUploader.tsx   # 多图上传（react-dropzone），base64 上传 GCS，回调 (publicUrl, file)，支持外部 loading
├── lib/
│   ├── types.ts            # Project / Page / GenerationTask 等类型
│   ├── gcs.ts              # GCS：generateUploadSignedUrl、uploadBuffer、deleteFile
...
│   ├── vision-python.ts    # 请求 VISION_SERVICE_URL/analyze_batch，FormData images[]
│   ├── vision.ts           # Google Cloud Vision 封装（当前未用，分析走 Python 服务）
│   └── pdf-fonts.ts        # 注册 Noto Sans SC 供 @react-pdf/renderer 中文 PDF
├── vision-service/        # Python 视觉服务（需单独启动）
│   ├── vision_service.py   # Flask：/health、/analyze、/analyze_batch，CLIP 零样本分类，返回中文标签
│   ├── requirements.txt   # flask, flask-cors, pillow, torch, torchvision, transformers
│   ├── Dockerfile          # 镜像构建
│   └── README.md           # 安装与启动说明
├── .env.local             # 本地环境变量（不提交）
├── Dockerfile              # Next 应用镜像
├── docker-compose.yml     # vision + app 双服务，app 依赖 vision 健康
├── DEPLOYMENT.md          # 部署说明（Docker、环境变量、GCS 密钥）
...
- **流程**：
  1. **第一步**：`PhotoUploader` 多图上传（最多 5 张），每张先 base64 上传到 GCS 得到 `photoUrls`，同时保留 `photoFiles`。已选图片以**缩略图网格**（约 80×80px）展示。用户点击「下一步：分析照片」时，将 `photoFiles` 以 `FormData`（字段 `images[]`）POST 到 `/api/analyze-images`。
  2. **分析失败**：若接口返回 503（视觉服务未启动），前端 alert 提示先启动 vision-service。
...
| 路径 | 方法 | 作用 | 依赖 |
|------|------|------|------|
| `/api/upload-file` | POST | body: filename, contentType, fileBase64 → 上传 GCS，返回 publicUrl | GCS 密钥、bucket |
...
| GCS_KEY_FILE | 项目根目录下 GCS 密钥 JSON 文件名（如 gcs-key.json） |
| GCS_BUCKET_NAME | GCS 桶名 |
| VISION_SERVICE_URL | Python 视觉服务地址，默认 http://localhost:5001 |
| NEXT_PUBLIC_BASE_URL | 前端用，如 http://localhost:3000 |
| GOOGLE_APPLICATION_CREDENTIALS | 可选，原 Google Vision 用；当前分析走 Python |
...
3. **视觉服务强依赖**：分析接口无降级，视觉服务未启动则 503；可考虑「可选分析」或友好引导页。
...
8. **安全与配置**：API Key 在服务端；GCS 桶若公开读需注意策略；未做请求限流与防滥用。
9. **首页与品牌**：hero 图需自行替换；无 SEO 结构化、无分享图；步骤说明可更贴合实际流程（如强调「先启动视觉服务」）。
